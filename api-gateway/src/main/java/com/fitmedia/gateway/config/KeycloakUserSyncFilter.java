package com.fitmedia.gateway.config;

import com.fitmedia.gateway.user.RegisterRequest;
import com.fitmedia.gateway.user.UserService;
...
public class KeycloakUserSyncFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain){
        ...
        if(userId != null && token != null){
            ...
            return userService.validadeUser(userId)
                    .flatMap(exist -> {
                        if(!exist){
                            // Cadastra usuário
                            if(request != null){
    ...
    private RegisterRequest getUserDetails(String token) {
        try{
            ...
            request.setPassword("senha@123123");
